...
#include <test/test-models/no-main/mcmc/ensemble/bernoulli.cpp>
#include <gtest/gtest.h>

typedef boost::ecuyer1988 rng_t;
typedef bernoulli_model_namespace::bernoulli_model Model;
...
TEST(McmcEnsembleBaseEnsemble, bernoulli_unconstrain_params) {
  Eigen::VectorXd a(1);
  a << 0.4;
  sampler.initialize_ensemble();
  Eigen::VectorXd res = sampler.unconstrain_params(a);
  EXPECT_FLOAT_EQ(std::log(0.4 / 0.6), res(0));
}
